#!/usr/bin/env python3
"""
Math Markdown to PNG Converter - Final Definitive Version

This version fixes a TypeError in the page.evaluate call by removing the
unsupported 'timeout' keyword argument.
"""
...
# --- BUNDLE FIX: Tell Playwright where to find the browser ---
if getattr(sys, 'frozen', False):
    browsers_path = os.path.join(sys.prefix, 'Resources', 'ms-playwright')
...
class PlaywrightMathConverter:
    def __init__(self, root):
        self.root = root
        self.root.title("Math Markdown to PNG Converter (Final)")
    ...
    def copy_to_clipboard(self):
        ...
        def _perform_copy(image):
            ...
            try:
                ...
                subprocess.run(
                    ['osascript', '-e', f'set the clipboard to (read (POSIX file "{tmp_file_path}") as TIFF picture)'],
                    check=True
                )
